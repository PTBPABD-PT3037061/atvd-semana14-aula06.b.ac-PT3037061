...
GRANT SELECT ON dbo.instructor TO User_B;
DENY SELECT ON dbo.instructor(salary) TO User_B;

GRANT SELECT ON dbo.takes TO User_B;
DENY SELECT ON dbo.takes(grade) TO User_B;
...
GRANT SELECT ON dbo.section (course_id, sec_id, semester, year) TO User_C;

GRANT UPDATE ON dbo.section (course_id, sec_id, semester, year) TO User_C;
...
-- CREATE OR ALTER VIEW dbo.vw_student_civil_eng AS
-- SELECT * FROM dbo.student 
-- WHERE dept_name = 'Civil Eng.';


GRANT SELECT ON dbo.vw_student_civil_eng TO User_E;


-- Questão 05. Revoque os privilégios do usuário User_E
-- Revogando a permissão que foi concedida na questão 04
REVOKE SELECT ON dbo.vw_student_civil_eng FROM User_E;
